...
module Web.StaticAPISpec (main, spec) where
...
withStaticAPI a =
    with (staticAPI a >> return (staticApp (defaultFileServerSettings "public"))) .
...
spec = do
    ...
    describe "StaticResponse" $ do
        withStaticAPI (route "path" $ return (object ["bool" .= True])) $
            it "encodes the static response with its JSON representation" $
                get "/path" `shouldRespondWith` "{\"bool\":true}" { matchStatus = 200 }

        describe "pathSegments" $ do
            withStaticAPI (route ("path1" ./ "path2") pathSegments) $
            ...
            withStaticAPI (route ("path" ./ ["path1", "path2"]) pathSegments) $
        ...
        describe "getPathSegment" $
            withStaticAPI (route ("name" .> ["path1", "path2"]) $ getPathSegment "name") $
